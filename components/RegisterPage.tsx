...
const registerSchema = z
  .object({
    ...
    username: z.preprocess(
      (val) => (typeof val === "string" ? val : ""),
      z.string().nonempty("Không được để trống").min(3, "Tên người dùng phải có ít nhất 3 ký tự")
    ),
    password: z.preprocess(
      (val) => (typeof val === "string" ? val : ""),
      z.string().nonempty("Không được để trống").min(6, "Mật khẩu phải có ít nhất 6 ký tự")
    ),
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flex: 1, justifyContent: "center" }} className="p-4">
            <View className="w-full max-w-md mx-auto">
              <View className="bg-white/90 backdrop-blur-md border border-blue-200/50 rounded-2xl shadow-xl px-6 pb-4">
                <View className="items-center space-y-4 mb-2">
                  <Image
                    source={require("@/assets/images/Logo_ZenKoi.png")}
                    className="w-32 h-32"
                  />
                  <Text className="text-2xl font-bold text-foreground text-center">Tạo tài khoản</Text>
                ...
                <Controller
                  ...
                  render={({ field: { onChange, value } }) => (
                    <>
                      <TextInput
                        className="bg-input border border-border rounded-lg px-4 py-3 mb-1"
                ...
                {/* Username */}
                <Text className="font-medium mb-2">Họ và tên</Text>
                <Controller
                  control={control}
                  name="username"
                  render={({ field: { onChange, value } }) => (
                ...
                {/* Confirm Password */}
                <Text className="font-medium mb-2">Xác nhận mật khẩu</Text>
                <Controller
                  control={control}
                  ...
                  render={({ field: { onChange, value } }) => (
                    <>
                      <View className="relative mb-1">
                        ...
                        />
                        <TouchableOpacity className="absolute right-3 top-3" onPress={() => setShowConfirmPassword(!showConfirmPassword)}>
                ...
                {/* Submit Button */}
                <TouchableOpacity className="bg-primary rounded-lg py-4 items-center mt-1" onPress={handleSubmit(onSubmit)} disabled={isLoading}>
                  {isLoading ? <ActivityIndicator color="#fff" /> : <Text className="text-white font-medium">Đăng ký</Text>}
                </TouchableOpacity>

                {/* Link Sign in */}
                <View className="flex-row justify-center mt-4">
                  <Text className="text-sm text-muted-foreground">Đã có tài khoản?</Text>
                  <TouchableOpacity onPress={() => router.push("/(auth)/login")}> 
                    <Text className="text-primary text-sm ml-1 font-medium">Đăng nhập</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </View>
